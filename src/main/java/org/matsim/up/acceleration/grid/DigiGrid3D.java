 ...
 * the centroids of the polyhedra that is used for the accelerometer research. 
 * Associated with the grid is the number of observations in each
...
public abstract class DigiGrid3D extends AbstractAnalysis{
	...
	static float BOUNDING_BOX_X_MIN = -700f;
	static float BOUNDING_BOX_X_MAX = 700f;
	static float BOUNDING_BOX_Y_MIN = -800f;
	static float BOUNDING_BOX_Y_MAX = 800f;
	static float BOUNDING_BOX_Z_MIN = 600f;
	static float BOUNDING_BOX_Z_MAX = 1500f;
	...
	/**
	 * (Re)setting the extent, or bounding box, of the visual. 
	 * 
	 * @param array
	 */
	public void setVisualBoundingBox(float[] array) {
		LOG.warn("(Re)setting the visual bounding box.");
		LOG.warn("Check that the given float[] is organised as [xMin, xMax, yMin, yMax, zMin, zMax]");
		BOUNDING_BOX_X_MIN = array[0];
		BOUNDING_BOX_X_MAX = array[1];
		BOUNDING_BOX_Y_MIN = array[2];
		BOUNDING_BOX_Y_MAX = array[3];
		BOUNDING_BOX_Z_MIN = array[4];
		BOUNDING_BOX_Z_MAX = array[5];
	}
	 ...
	 * 	<li><b>SLICE</b>: takes a horizontal (xy) slice at a given z-value;
	...
	public void init() throws Exception {
		...
		case NONE:
			/* Do nothing */
			LOG.warn("Not visualising anything.");
			break;
		case CENTROID:
			printCentroids();
			break;
